...
test_init_output_format() {
  ...
  judge_criterion "ralph init prints a per-artifact summary on exit: a leading '✓ Bootstrapped wrapix project in .' banner, a 'Created:' section listing each artifact the run produced (including flake.nix and nix/flake/ as separate top-level entries on the flake create path, .envrc, .gitignore with '(N entries appended)' detail, .pre-commit-config.yaml, docs/README.md, docs/architecture.md, docs/style-guidelines.md, AGENTS.md, CLAUDE.md with '(-> AGENTS.md)' detail, and .wrapix/ralph/template/), a 'Skipped:' section listing each artifact that was left alone with a parenthetical reason (e.g. '.beads/  (already initialized)'), and a fixed two-step 'Next steps:' block — 'direnv allow' then 'ralph plan -n <label>' — followed by a 'Docs: specs/ralph-harness.md' pointer. On a re-run where flake.nix already exists, only 'flake.nix  (already exists)' appears in 'Skipped:' — no per-module nix/flake/ noise. The next-steps block is fixed (always the same two items + docs pointer) regardless of what was created or skipped."
}
