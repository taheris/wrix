...
test_rust_profile_no_nightly_closure() {
  local fixture_sha="sha256-SXRtAuO4IqNOQq+nLbrsDFbVk+3aVA8NNpSZsKlVH/8="
  local cwd flake_url fixture_path
  cwd=$(pwd)
  flake_url="git+file://$cwd"
  fixture_path="$cwd/tests/fixtures/rust-toolchain.toml"

  if [ ! -f "$fixture_path" ]; then
    judge_assert FAIL "fixture not found: $fixture_path"
    return
  fi

  local system
  if ! system=$(nix eval --raw --impure --no-warn-dirty --expr 'builtins.currentSystem'); then
    judge_assert FAIL "nix eval builtins.currentSystem failed (see stderr)"
    return
  fi
  ...
  "); then
    judge_assert FAIL "nix eval for default rust toolchain drvPath failed (see stderr)"
    return
  fi

  local default_count
  default_count=$(nix-store -qR "$default_drv" | awk 'tolower($0) ~ /nightly/ {n++} END {print n+0}')
  if [ "$default_count" -ne 0 ]; then
    judge_assert FAIL "default rust toolchain closure contains $default_count nightly-* derivation(s) — likely regression to fenix.packages.\${system}.rust-analyzer"
    return
  fi
  ...
  "); then
    judge_assert FAIL "nix eval for withToolchain rust toolchain drvPath failed (see stderr)"
    return
  fi

  local withtc_count
  withtc_count=$(nix-store -qR "$withtc_drv" | awk 'tolower($0) ~ /nightly/ {n++} END {print n+0}')
  if [ "$withtc_count" -ne 0 ]; then
    judge_assert FAIL "withToolchain rust toolchain closure contains $withtc_count nightly-* derivation(s) — likely regression to fenix.packages.\${system}.rust-analyzer"
    return
  fi

  judge_assert PASS "default and withToolchain rust toolchain closures contain zero nightly-* derivations"
}
