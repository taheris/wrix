...
$(cd "$workspace" && "$wrix_bin" service cache publish)"
  ...
  [[ -f "$state/publish-roots.json" ]] || fail "publish manifest was not refreshed"
  assert_contains "publish manifest" "$(cat "$state/publish-roots.json")" "\"drv_path\": \"/nix/store/11111111111111111111111111111111-pkg.drv\""
  assert_contains "publish manifest" "$(cat "$state/publish-roots.json")" "\"drv_path\": \"/nix/store/22222222222222222222222222222222-check.drv\""
