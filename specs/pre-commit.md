...
- **Mechanism** — versioned shims at `lib/prek/hooks/pre-commit` and `lib/prek/hooks/pre-push` acquire the `flock` and then `exec` prek's git-hook entry point (`prek hook-impl --hook-dir "$(dirname "$0")" --script-version 4 --hook-type=<stage> -- "$@"`). `hook-impl` is used rather than `prek run` because git passes stage-specific positional args (pre-push receives `<remote-name> <remote-url>`, commit-msg receives the message path) that `prek run` would mistake for hook/project selectors; `hook-impl` treats them as git metadata and reads pushed refs from stdin. `--script-version 4` matches the version expected by the currently pinned prek and silences the "outdated shim" warning. Git is pointed at the shim directory via `core.hooksPath = lib/prek/hooks`, so the shims are the source of truth rather than generated artifacts under `.git/hooks/`. Shims share a single helper (`lib/prek/hooks/_flock.sh`) so the per-stage scripts stay one-liners
