...
- The `agentPkg` code path threads the selected agent package into the image build
  [system](nix run .#test-agent-pkg-threaded)
...
- Every wrix-managed Nix-built image source covered by this spec exposes the platform source kind (`nix-descriptor` on Linux, `docker-archive` on Darwin), and the verifier enumerates the explicit `wrix-builder` exemption owned by `linux-builder.md`
