...
The descriptor/config metadata exposes the image digest consumed by the runtime install preflight owned by `sandbox.md`. Computing that metadata does not execute the image source, materialize a whole-image tar, or run a Docker-archive-to-OCI conversion. The provenance-tiered graph keeps unchanged lower-tier layer digests byte-identical across image variants. The descriptor remains reproducible: given the same profile, selected agent, agent package, generated settings, and MCP config, it produces the same layer digest list and config digest.
