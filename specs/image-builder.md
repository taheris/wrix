...
Chaining alone does not deduplicate. `dockerTools` does not subtract the `fromImage` closure from the per-profile layering graph — it only reserves a layer-count budget for the base. Left to its default, the per-profile builder layers the full profile closure (which still closes over glibc, gcc-lib, openssl, the shells, coreutils) and re-emits every base store path as duplicate top layers, inflating each profile image and defeating the load-once goal. `mkImage` therefore drives the layering with an explicit `layeringPipeline` that `remove_paths` the base closure — the closure of `base-contents.nix`, exactly the store paths the base layers ship — before the `popularity_contest` + `limit_layers` split. The removed paths still resolve at runtime from the base's lower layers, so the rootfs is unchanged while the duplication is gone. `limit_layers` (`maxStoreLayers`) bounds only genuinely-new content; with the base's layers and the customisation layer added on top, the total stays below the 127-layer OCI ceiling.
...
- The per-profile layering removes the `wrapix-base-image` closure from its graph via `layeringPipeline`, so base store paths are not re-emitted as duplicate top layers atop `fromImage`
  [check](grep -nE 'remove_paths|baseClosurePaths' lib/sandbox/image.nix)
