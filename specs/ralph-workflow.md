...
**SUMMARY column** is sourced from the `## Options — <summary>` header of the clarify bead (see Options Format Contract in the `ralph:clarify` Label section). Fallback when the header is absent or has no summary text: the bead title.
...
| `lib/ralph/template/msg.md` | Interactive Drafter session prompt for `ralph msg` (clarify-resolution) |
...
Once the requirements added in this round (anchor-driven multi-spec planning, per-spec cursor fan-out, `ralph init`, `ralph msg` redesign with Options Format Contract) are implemented and green, this spec is to be **mechanically refactored** into the following structure. No new requirements are introduced by the split; it's pure reorganization.

| Target spec | Source sections |
|-------------|-----------------|
| `specs/ralph-harness.md` | Problem statement, phase diagram (top-level), state management (per-label JSON, `state/current`, spec resolution, anchor & sibling state files), compaction re-pin, template system (Nix definitions, partials, variables, `.wrapix/ralph/config.nix`, flake check integration), template content requirements for **partials only**, utility commands (`use`, `status`, `logs`, `sync`, `tune`, `init`), container execution table, `ralph:clarify` label definition |
...
| `specs/ralph-review.md` (new, renamed from prior `ralph-check.md` proposal) | `ralph check` + `ralph msg` commands; template content for `check.md` and `msg.md`; invariant-clash review + three-paths principle; **Options Format Contract** (`## Options — <summary>` / `### Option N — <title>`) — consumer-owned by `msg`, producer-conformed by `check`; push gate + auto-iteration; iteration cap + escalation; push failure handling; container bead sync protocol (for check and interactive msg) |
...
| [ralph-review.md]     (new)| [`lib/ralph/cmd/{check,msg}.sh`]        | <new>   | Review gate: invariant clash, options format, push gate, clarify resolution |
```

The split should NOT be attempted before multi-spec awareness ships — current single-spec `ralph todo` cannot see across spec files, and a split ahead of multi-spec implementation would produce tasks that only reference the surviving overview spec, missing content migrated to `ralph-loop.md` and `ralph-review.md`. The split is a pure mechanical refactor; it should land before the `ralph msg` redesign implementation so that new work targets `ralph-review.md` directly. The `ralph msg` redesign requirements in this spec are carried over to `ralph-review.md` by the split, not introduced by it.
