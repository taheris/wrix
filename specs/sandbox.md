...
- Pi images seed `defaultModel = "gpt-6-astra"`, `editorPaddingX = 1`, and `enableInstallTelemetry = false`, so GPT-6 Astra is selected by default, the input editor has one cell of horizontal padding, and Pi's anonymous install/update ping plus optional provider attribution headers are disabled by default; update checking remains a separate Pi setting.
  [check](test-ci:test-sandbox-agent-settings)
