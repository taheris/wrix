...
| Profile Config | Immutable JSON config for the Rust launcher: image, agent, mounts, env, network allowlist, service features | `ProfileConfig` generated by `mkSandbox` |
...
| `packages.profile-images` | JSON manifest mapping profile → profile config/image metadata, for orchestrators that look up images by profile name |
...
- `wrix spawn --spawn-config <file> [--stdio]` — programmatic dispatch. Reads the same `ProfileConfig` plus per-launch workspace, env allowlist, image override, mounts, and agent args from JSON `SpawnConfig`. `--stdio` adds `WRIX_STDIO=1` so the selected agent uses its stdio protocol (`claude --input-format stream-json`, `pi --mode rpc`).
