...
# selected agent runtime and its closure — `claude-code` (claude), the
# consumer-supplied `piPkg` (pi), or `directRunner` (direct); a non-selected
# agent's binary is absent. Chained atop wrapix-stable-profile-<name>, with the
